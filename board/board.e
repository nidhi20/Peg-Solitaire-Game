...
feature -- Constructor
	make_default
		...
		do
			...
			board_set:
				imp.is_equal (create {ARRAY2[SLOT_STATUS]}.make_filled (unavailable_slot,7,7))
	...
	: BOOLEAN
		...
		require
			consistent_row_numbers:
				other.number_of_rows = Current.number_of_rows
			consistent_column_numbers:
				other.number_of_columns = Current.number_of_columns
			valid_rows:
				other.is_valid_row (r1) and other.is_valid_row (r2)
			and	Current.is_valid_row (r1) and Current.is_valid_row (r2)
			valid_columns:
				other.is_valid_column (c1) and other.is_valid_column (c2)
			and	Current.is_valid_column (c1) and Current.is_valid_column (c2)
			valid_row_range:
				r2 >= r1
			valid_column_range:
				c2 >= c1
		do
			-- we need to compare eleents one operation at  time, except for the one in the given range, then return
			-- Only returns true if the status matches, and they are outside the matching criterion
			Result :=
			across 1 |..| number_of_rows as i
			all
				across 1 |..| number_of_columns as j
				all
...
feature -- Queries
	...
	is_valid_row (r: INTEGER): BOOLEAN
		...
		do
			Result := 1 <= r and r <= imp.height
		ensure
			correct_result:
				Result implies (1 <= r and r <= imp.height)
		end

	is_valid_column (c: INTEGER): BOOLEAN
			-- Is 'x' a valid column number?
		do
			Result := 1 <= c and c <= imp.width
		ensure
			correct_result: Result implies (1 <= c and c <= imp.width)
		end
	...
	number_of_occupied_slots: INTEGER
		...
		do
			...
			loop
				across 1 |..| number_of_columns as j
				loop
					if status_of(i.item,j.item) = occupied_slot then
						Result := Result + 1
					end
			...
			across 1 |..| number_of_rows as i
				loop
					across 1 |..| number_of_columns as j
						loop
							if status_of (i.item,j.item) = occupied_slot then
								Result.append(occupied_slot.out) -- this ensures implementation invariant ouput
							elseif status_of (i.item,j.item) = unoccupied_slot then
								Result.append(unoccupied_slot.out)
							elseif status_of (i.item,j.item) = unavailable_slot then
								Result.append(unavailable_slot.out)
							end
						end

					if i.item < number_of_rows then
